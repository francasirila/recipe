...
class TheMealDBProvider(ExternalRecipeProvider): 
    def __init__(self): 
        self.base_url = os.getenv( "THEMEALDB_BASE_URL", "https://www.themealdb.com/api/json/v1/1" ) 
    ...
    async def get_by_id( self, recipe_id: str ): 
        data = await self._get("lookup.php", {"i": recipe_id}) 
        meals = data.get("meals") or [] 
        
        if not meals: 
            return None 
        return self._transform_meal( meals[0] ) 
